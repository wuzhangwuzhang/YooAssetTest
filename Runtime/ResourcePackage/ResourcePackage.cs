...
namespace YooAsset
{
    public class ResourcePackage
    {
        ...
        {
            if (_isInitialize && InitializeStatus == EOperationStatus.Failed)
            {
                _isInitialize = false;
                InitializeStatus = EOperationStatus.None;
                _initializeError = string.Empty;
        ...
        public UpdatePackageManifestOperation UpdatePackageManifestAsync(string packageVersion, int timeout = 60)
        {
            ...
            if (_resourceManager.HasAnyLoader())
                YooLogger.Warning(
                    $"Found loaded bundle before update manifest ! Recommended to call the  {nameof(UnloadAllAssetsAsync)} method to release loaded bundle !");
        ...
        public ResourceImporterOperation CreateResourceImporter(string[] filePaths, int importerMaxNumber,
            int failedTryAgain)
        {
            DebugCheckInitialize();
        ...
        /// <param name="suspendLoad">场景加载到90%自动挂起</param>
        /// <param name="priority">加载的优先级</param>
        public SceneHandle LoadSceneAsync(AssetInfo assetInfo, LoadSceneMode sceneMode = LoadSceneMode.Single,
            LocalPhysicsMode physicsMode = LocalPhysicsMode.None, bool suspendLoad = false, uint priority = 0)
        {
            DebugCheckInitialize();
            return LoadSceneInternal(assetInfo, false, sceneMode, physicsMode, suspendLoad, priority);
        ...
        private SceneHandle LoadSceneInternal(AssetInfo assetInfo, bool waitForAsyncComplete, LoadSceneMode sceneMode,
            LocalPhysicsMode physicsMode, bool suspendLoad, uint priority)
        {
        ...
        {
            DebugCheckInitialize();
            return _playModeImpl.CreateResourceDownloaderByTags(new[] { tag }, downloadingMaxNumber, failedTryAgain,
                timeout);
        }
        ...
        /// <param name="failedTryAgain">下载失败的重试次数</param>
        /// <param name="timeout">超时时间</param>
        public ResourceDownloaderOperation CreateBundleDownloader(string location, int downloadingMaxNumber,
            int failedTryAgain, int timeout = 60)
        {
            DebugCheckInitialize();
            var assetInfo = ConvertLocationToAssetInfo(location, null);
            AssetInfo[] assetInfos = { assetInfo };
            return _playModeImpl.CreateResourceDownloaderByPaths(assetInfos, downloadingMaxNumber, failedTryAgain,
                timeout);
        }
        ...
        /// <param name="failedTryAgain">下载失败的重试次数</param>
        /// <param name="timeout">超时时间</param>
        public ResourceDownloaderOperation CreateBundleDownloader(string[] locations, int downloadingMaxNumber,
            int failedTryAgain, int timeout = 60)
        {
            DebugCheckInitialize();
            ...
            return _playModeImpl.CreateResourceDownloaderByPaths(assetInfos.ToArray(), downloadingMaxNumber,
        ...
        /// <param name="failedTryAgain">下载失败的重试次数</param>
        /// <param name="timeout">超时时间</param>
        public ResourceDownloaderOperation CreateBundleDownloader(AssetInfo assetInfo, int downloadingMaxNumber,
            int failedTryAgain, int timeout = 60)
        {
            DebugCheckInitialize();
            AssetInfo[] assetInfos = { assetInfo };
        ...
        {
            DebugCheckInitialize();
            return _playModeImpl.CreateResourceUnpackerByTags(new[] { tag }, unpackingMaxNumber, failedTryAgain,
                int.MaxValue);
        }
        ...
        private void DebugCheckAssetLoadType(Type type)
        {
            ...
            if (typeof(Object).IsAssignableFrom(type) == false)
                throw new Exception($"Load asset type is invalid : {type.FullName} !");
        }
