        ...
        internal override void InternalOnUpdate()
        {
            ...
            try
            {
                ...
                if (_steps == ESteps.DeserializeAssetList)
                {
                    while (_packageAssetCount > 0)
                    {
                        ...
                        // 填充AssetPathMapping1
                        {
                            ...
                            if (Manifest.AssetPathMapping1.ContainsKey(location))
                                throw new Exception($"Location have existed : {location}");
                        ...
                        if (Manifest.IncludeAssetGUID)
                        {
                            if (Manifest.AssetPathMapping2.ContainsKey(packageAsset.AssetGUID))
                                throw new Exception($"AssetGUID have existed : {packageAsset.AssetGUID}");
                    ...
                    if (_packageAssetCount <= 0) _steps = ESteps.PrepareBundleList;
                }
