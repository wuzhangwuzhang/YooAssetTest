...
namespace YooAsset
{
    internal static class ManifestTools
    {
        ...
        ///     获取远端的资源文件名
        /// </summary>
        public static string GetRemoteBundleFileName(int nameStyle, string bundleName, string fileExtension,
            string fileHash)
        {
            if (nameStyle == (int)EFileNameStyle.HashName)
                return StringUtility.Format("{0}{1}", fileHash, fileExtension);

            if (nameStyle == (int)EFileNameStyle.BundleName) return bundleName;

            if (nameStyle == (int)EFileNameStyle.BundleName_HashName)
            {
        ...
        public static PackageManifest DeserializeFromBinary(byte[] binaryData)
        {
            ...
            if (fileVersion != YooAssetSettings.ManifestFileVersion)
                throw new Exception(
                    $"The manifest file version are not compatible : {fileVersion} != {YooAssetSettings.ManifestFileVersion}");
