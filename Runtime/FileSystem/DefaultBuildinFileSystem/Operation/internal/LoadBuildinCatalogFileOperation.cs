        ...
        internal override void InternalOnUpdate()
        {
            ...
            if (_steps == ESteps.LoadCatalog)
            {
                ...
                if (catalog.PackageName != _fileSystem.PackageName)
                {
                    ...
                    Status = EOperationStatus.Failed;
                    Error =
                        $"Catalog file package name {catalog.PackageName} cannot match the file system package name {_fileSystem.PackageName}";
                    return;
                }
                ...
                YooLogger.Log(
                    $"Package '{_fileSystem.PackageName}' buildin catalog files count : {catalog.Wrappers.Count}");
                _steps = ESteps.Done;
