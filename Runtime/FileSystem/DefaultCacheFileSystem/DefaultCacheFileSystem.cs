...
namespace YooAsset
{
    ...
    internal class DefaultCacheFileSystem : IFileSystem
    {
        ...
        public virtual void OnUpdate()
        {
            ...
            foreach (var key in _removeList) _downloaders.Remove(key);
        }
        ...
        ///     自定义参数：断点续传下载器关注的错误码
        /// </summary>
        public List<long> ResumeDownloadResponseCodes { private set; get; }
        ...
        protected string GetDataFilePath(PackageBundle bundle)
        {
            if (_dataFilePaths.TryGetValue(bundle.BundleGUID, out var filePath) == false)
            {
                ...
                filePath = PathUtility.Combine(_saveFileRoot, folderName, bundle.BundleGUID,
                    DefaultCacheFileSystemDefine.SaveBundleDataFileName);
        ...
        protected string GetInfoFilePath(PackageBundle bundle)
        {
            if (_infoFilePaths.TryGetValue(bundle.BundleGUID, out var filePath) == false)
            {
                var folderName = bundle.FileHash.Substring(0, 2);
                filePath = PathUtility.Combine(_saveFileRoot, folderName, bundle.BundleGUID,
                    DefaultCacheFileSystemDefine.SaveBundleInfoFileName);
        ...
        public EFileVerifyResult VerifyCacheFile(PackageBundle bundle)
        {
            ...
            var result = FileSystemHelper.FileVerify(wrapper.DataFilePath, wrapper.DataFileSize, wrapper.DataFileCRC,
        ...
        {
            if (_wrappers.ContainsKey(bundle.BundleGUID)) throw new Exception("Should never get here !");
        ...
        {
            if (Directory.Exists(_manifestFileRoot)) Directory.Delete(_manifestFileRoot, true);
        }
