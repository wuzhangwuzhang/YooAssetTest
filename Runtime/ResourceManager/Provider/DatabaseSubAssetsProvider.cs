...
namespace YooAsset
{
    ...
    {
        public DatabaseSubAssetsProvider(ResourceManager manager, string providerGUID, AssetInfo assetInfo) : base(
        ...
        internal override void InternalOnUpdate()
        {
            ...
            if (_steps == ESteps.Loading)
            {
                ...
                {
                    AllAssetObjects = AssetDatabase.LoadAllAssetRepresentationsAtPath(MainAssetInfo.AssetPath);
                }
                else
                {
                    var findAssets = AssetDatabase.LoadAllAssetRepresentationsAtPath(MainAssetInfo.AssetPath);
            ...
            if (_steps == ESteps.Checking)
            {
                if (AllAssetObjects == null)
                {
                    ...
                    if (MainAssetInfo.AssetType == null)
                        ...
                        error =
                            $"Failed to load sub assets : {MainAssetInfo.AssetPath} AssetType : {MainAssetInfo.AssetType}";
                    YooLogger.Error(error);
                    InvokeCompletion(error, EOperationStatus.Failed);
                }
                else
