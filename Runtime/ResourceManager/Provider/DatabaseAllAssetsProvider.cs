...
namespace YooAsset
{
    ...
    {
        public DatabaseAllAssetsProvider(ResourceManager manager, string providerGUID, AssetInfo assetInfo) : base(
        ...
        internal override void InternalOnUpdate()
        {
            ...
            if (_steps == ESteps.Loading)
            {
                ...
                else
                {
                    ...
                    foreach (var assetPath in LoadBundleFileOp.BundleFileInfo.IncludeAssetsInEditor)
                    {
                        var mainAsset = AssetDatabase.LoadAssetAtPath(assetPath, MainAssetInfo.AssetType);
                        if (mainAsset != null)
                            result.Add(mainAsset);
                    }
            ...
            if (_steps == ESteps.Checking)
            {
                if (AllAssetObjects == null)
                {
                    ...
                    if (MainAssetInfo.AssetType == null)
                        ...
                        error =
                            $"Failed to load all assets : {MainAssetInfo.AssetPath} AssetType : {MainAssetInfo.AssetType}";
                    YooLogger.Error(error);
                    InvokeCompletion(error, EOperationStatus.Failed);
                }
                else
