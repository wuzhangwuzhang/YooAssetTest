    ...
    public sealed class UnloadAllAssetsOperation : AsyncOperationBase
    {
        ...
        internal override void InternalOnUpdate()
        {
            ...
            if (_steps == ESteps.AbortDownload)
            {
                ...
                foreach (var loader in loaderDic.Values) loader.AbortDownloadOperation();
            ...
            if (_steps == ESteps.UnloadAll)
            {
                ...
                // 释放所有资源句柄
                foreach (var provider in providerDic.Values) provider.ReleaseAllHandles();

                // 强制销毁资源提供者
                foreach (var provider in providerDic.Values) provider.DestroyProvider();

                // 强制销毁文件加载器
                foreach (var loader in loaderDic.Values) loader.DestroyLoader();
