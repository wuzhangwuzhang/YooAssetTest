...
namespace YooAsset.Editor
{
    ...
    public class AssetBundleCollector
    {
        ...
        public bool IsValid()
        {
            if (AssetDatabase.LoadAssetAtPath<Object>(CollectPath) == null)
                return false;
        ...
        {
            var assetGUID = AssetDatabase.AssetPathToGUID(CollectPath);
        ...
        public bool FixConfigError()
        {
            ...
            {
                var convertAssetPath = AssetDatabase.GUIDToAssetPath(CollectorGUID);
        ...
        public List<CollectAssetInfo> GetAllCollectAssets(CollectCommand command, AssetBundleCollectorGroup group)
        {
            ...
            var result = new Dictionary<string, CollectAssetInfo>(1000);

            // 收集打包资源路径
            var findAssets = new List<string>();
            if (AssetDatabase.IsValidFolder(CollectPath))
            {
                var collectDirectory = CollectPath;
                var findResult = EditorTools.FindAssets(EAssetSearchType.All, collectDirectory);
                findAssets.AddRange(findResult);
            }
            else
            {
                var assetPath = CollectPath;
                ...
                foreach (var collectInfoPair in result)
                    if (collectInfoPair.Value.CollectorType == ECollectorType.MainAssetCollector)
                    {
                        ...
                        if (addressTemper.TryGetValue(address, out var existed) == false)
                            ...
                            throw new Exception(
                                $"The address is existed : {address} in collector : {CollectPath} \nAssetPath:\n     {existed}\n     {assetPath}");
                    }
            }
        ...
        private CollectAssetInfo CreateCollectAssetInfo(CollectCommand command, AssetBundleCollectorGroup group,
            AssetInfo assetInfo)
        {
            var address = GetAddress(command, group, assetInfo);
            var bundleName = GetBundleName(command, group, assetInfo);
            var assetTags = GetAssetTags(group);
            var collectAssetInfo = new CollectAssetInfo(CollectorType, bundleName, address, assetInfo, assetTags);
        ...
        private bool IsCollectAsset(AssetBundleCollectorGroup group, AssetInfo assetInfo)
        {
            ...
            return filterRuleInstance.IsCollectAsset(new FilterRuleData(assetInfo.AssetPath, CollectPath,
                group.GroupName, UserData));
        }
        ...
        private string GetAddress(CollectCommand command, AssetBundleCollectorGroup group, AssetInfo assetInfo)
        {
            ...
            var addressRuleInstance = AssetBundleCollectorSettingData.GetAddressRuleInstance(AddressRuleName);
            var adressValue =
                addressRuleInstance.GetAssetAddress(new AddressRuleData(assetInfo.AssetPath, CollectPath,
                    group.GroupName, UserData));
            return adressValue;
        }
        ...
        private string GetBundleName(CollectCommand command, AssetBundleCollectorGroup group, AssetInfo assetInfo)
        {
            if (command.AutoCollectShaders)
                ...
                {
                    // 获取着色器打包规则结果
                    var shaderPackRuleResult = DefaultPackRule.CreateShadersPackRuleResult();
                    return shaderPackRuleResult.GetBundleName(command.PackageName, command.UniqueBundleName);
                }

            // 获取其它资源打包规则结果
            var packRuleInstance = AssetBundleCollectorSettingData.GetPackRuleInstance(PackRuleName);
            var defaultPackRuleResult =
                packRuleInstance.GetPackRuleResult(new PackRuleData(assetInfo.AssetPath, CollectPath, group.GroupName,
                    UserData));
        ...
        {
            var tags = EditorTools.StringToStringList(group.AssetTags, ';');
            var temper = EditorTools.StringToStringList(AssetTags, ';');
        ...
        {
            var depends = AssetDatabase.GetDependencies(mainAssetPath, true);
            var result = new List<AssetInfo>(depends.Length);
            foreach (var assetPath in depends)
            {
                // 注意：排除主资源对象
