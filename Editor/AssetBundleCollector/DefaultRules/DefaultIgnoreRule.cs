...
namespace YooAsset.Editor
{
    public class DefaultIgnoreRule
    {
        ...
        public static readonly HashSet<string> IgnoreFileExtensions =
            new() { "", ".so", ".dll", ".cs", ".js", ".boo", ".meta", ".cginc", ".hlsl" };
    }
    ...
    public class NormalIgnoreRule : IIgnoreRule
    {
        ...
        {
            if (assetInfo.AssetPath.StartsWith("Assets/") == false &&
                assetInfo.AssetPath.StartsWith("Packages/") == false)
            {
                Debug.LogError($"Invalid asset path : {assetInfo.AssetPath}");
                return true;
            }
            ...
            // 忽略Unity引擎无法识别的文件
            if (assetInfo.AssetType == typeof(DefaultAsset))
            {
                Debug.LogWarning($"Cannot pack default asset : {assetInfo.AssetPath}");
                return true;
            }
