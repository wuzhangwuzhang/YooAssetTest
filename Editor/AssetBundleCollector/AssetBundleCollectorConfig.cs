...
namespace YooAsset.Editor
{
    public class AssetBundleCollectorConfig
    {
        ...
        public static void ImportXmlConfig(string filePath)
        {
            ...
            // 读取配置版本
            var configVersion = root.GetAttribute(XmlVersion);
            ...
            // 读取公共配置
            var uniqueBundleName = false;
            var showPackageView = false;
            var showEditorAlias = false;
            var commonNodeList = root.GetElementsByTagName(XmlCommon);
            if (commonNodeList.Count > 0)
            {
                var commonElement = commonNodeList[0] as XmlElement;
            ...
            foreach (var packageNode in packageNodeList)
            {
                ...
                {
                    var groupElement = groupNode as XmlElement;
                    ...
                    {
                        var collectorElement = collectorNode as XmlElement;
                        ...
                        collector.CollectorType =
                            EditorTools.NameToEnum<ECollectorType>(collectorElement.GetAttribute(XmlCollectorType));
            ...
            // 检测配置错误
            foreach (var package in packages) package.CheckConfigError();
        ...
        public static void ExportXmlConfig(string savePath)
        {
            ...
            commonElement.SetAttribute(XmlShowPackageView,
                AssetBundleCollectorSettingData.Setting.ShowPackageView.ToString());
            commonElement.SetAttribute(XmlShowEditorAlias,
                AssetBundleCollectorSettingData.Setting.ShowEditorAlias.ToString());
            commonElement.SetAttribute(XmlUniqueBundleName,
                AssetBundleCollectorSettingData.Setting.UniqueBundleName.ToString());
