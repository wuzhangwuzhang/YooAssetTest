...
namespace YooAsset.Editor
{
    public class AssetBundleCollectorWindow : EditorWindow
    {
        ...
        private int _lastModifyGroupIndex;

        private int _lastModifyPackageIndex;
        ...
        private bool _showPackageSettings;
        ...
        {
            var resultPath = EditorTools.OpenFolderPanel("Export XML", "Assets/");
            if (resultPath != null)
        ...
        {
            var resultPath = EditorTools.OpenFilePath("Import XML", "Assets/", "xml");
            if (resultPath != null)
            {
        ...
        private void RefreshPackageSetting()
        {
            ...
            if (selectPackage != null)
            {
                var packageSettingName = "Package Settings";
        ...
        private void RefreshHelpBoxTips()
        {
            ...
            {
                var tips =
                    "The [Enable Addressable] option and [Location To Lower] option cannot be enabled at the same time.";
            ...
            {
                var tips =
                    "There are multiple Packages in the current config, Recommended to enable the [Unique Bundle Name] option.";
        ...
        private void BindGroupListViewItem(VisualElement element, int index)
        {
            ...
            var isActive = activeRule.IsActiveGroup();
        ...
        private void FillCollectorViewData()
        {
            ...
            for (var i = 0; i < selectGroup.Collectors.Count; i++)
            {
                var element = MakeCollectorListViewItem();
        ...
        private void BindCollectorListViewItem(VisualElement element, int index)
        {
            ...
            var collectObject = AssetDatabase.LoadAssetAtPath<Object>(collector.CollectPath);
            ...
            objectField1.RegisterValueChangedCallback(evt =>
            {
                ...
                if (foldout.value) RefreshFoldout(foldout, selectGroup, collector);
            });
        ...
        private void RefreshFoldout(Foldout foldout, AssetBundleCollectorGroup group, AssetBundleCollector collector)
        {
                ...
                collector.CollectorType == ECollectorType.StaticAssetCollector)
            {
                ...
                {
                    var ignoreRule =
                        AssetBundleCollectorSettingData.GetIgnoreRuleInstance(_ignoreRulePopupField.value.ClassName);
                ...
                if (collectAssetInfos != null)
                    foreach (var collectAsset in collectAssetInfos)
                    {
                        ...
                        var showInfo = collectAsset.AssetInfo.AssetPath;
                        if (_enableAddressableToogle.value)
        ...
        {
            for (var i = 0; i < _collectorTypeList.Count; i++)
        ...
        {
            for (var i = 0; i < _addressRuleList.Count; i++)
        ...
        {
            for (var i = 0; i < _packRuleList.Count; i++)
        ...
        {
            for (var i = 0; i < _filterRuleList.Count; i++)
        ...
        {
            for (var i = 0; i < _ignoreRuleList.Count; i++)
        ...
        {
            for (var i = 0; i < _activeRuleList.Count; i++)
