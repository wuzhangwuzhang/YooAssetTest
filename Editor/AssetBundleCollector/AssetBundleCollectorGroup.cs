...
namespace YooAsset.Editor
{
    ...
    public class AssetBundleCollectorGroup
    {
        ...
        public void CheckConfigError()
        {
            ...
            // 检测分组是否激活
            var activeRule = AssetBundleCollectorSettingData.GetActiveRuleInstance(ActiveRuleName);
            if (activeRule.IsActiveGroup() == false)
                return;

            foreach (var collector in Collectors) collector.CheckConfigError();
        }
        ...
        public List<CollectAssetInfo> GetAllCollectAssets(CollectCommand command)
        {
            ...
            // 检测分组是否激活
            var activeRule = AssetBundleCollectorSettingData.GetActiveRuleInstance(ActiveRuleName);
            if (activeRule.IsActiveGroup() == false) return new List<CollectAssetInfo>();
            ...
            foreach (var collector in Collectors)
            {
                ...
                foreach (var collectAsset in temper)
                    if (result.ContainsKey(collectAsset.AssetInfo.AssetPath) == false)
                        ...
                        throw new Exception(
                            $"The collecting asset file is existed : {collectAsset.AssetInfo.AssetPath} in group : {GroupName}");
            }
            ...
            if (command.EnableAddressable)
            {
                ...
                foreach (var collectAssetPair in result)
                    ...
                    {
                        var address = collectAssetPair.Value.Address;
                        var assetPath = collectAssetPair.Value.AssetInfo.AssetPath;
                        ...
                        if (addressTemper.TryGetValue(address, out var existed) == false)
                            ...
                            throw new Exception(
                                $"The address is existed : {address} in group : {GroupName} \nAssetPath:\n     {existed}\n     {assetPath}");
                    }
            }
