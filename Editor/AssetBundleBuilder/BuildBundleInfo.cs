...
namespace YooAsset.Editor
{
    public class BuildBundleInfo
    {
        ...
        ///     注意：不包含零依赖资源和冗余资源
        /// </summary>
        public readonly List<BuildAssetInfo> MainAssets = new();
        ...
        public void PackAsset(BuildAssetInfo buildAsset)
        {
            if (IsContainsAsset(buildAsset.AssetInfo.AssetPath))
                throw new Exception($"Should never get here ! Asset is existed : {buildAsset.AssetInfo.AssetPath}");
        ...
        public List<string> GetAllBuiltinAssetPaths()
        {
            ...
            var result = new List<string>(packAssets);
            foreach (var buildAsset in MainAssets)
            {
