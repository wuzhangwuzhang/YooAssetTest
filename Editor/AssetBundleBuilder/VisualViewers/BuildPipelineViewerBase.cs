...
namespace YooAsset.Editor
{
    internal abstract class BuildPipelineViewerBase
    {
        ...
        private void CreateView(VisualElement parent)
        {
            ...
            // 输出目录
            var defaultOutputRoot = AssetBundleBuilderHelper.GetDefaultBuildOutputRoot();
            ...
            // 构建模式
            {
                ...
                var defaultIndex = buildModeList.FindIndex(x => x.Equals(buildMode));
                if (defaultIndex < 0)
                ...
                {
                    AssetBundleBuilderSetting.SetPackageBuildMode(PackageName, BuildPipeline,
                        (EBuildMode)_buildModeField.value);
                });
            ...
            // 加密方法
            {
                ...
                if (encryptionClassTypes.Count > 0)
                {
                    ...
                    var defaultIndex = encryptionClassTypes.FindIndex(x => x.FullName.Equals(encyptionClassName));
                    if (defaultIndex < 0)
                        defaultIndex = 0;
        ...
        private void RefreshView()
        {
            var buildinFileCopyOption =
                AssetBundleBuilderSetting.GetPackageBuildinFileCopyOption(PackageName, BuildPipeline);
            var tagsFiledVisible = buildinFileCopyOption == EBuildinFileCopyOption.ClearAndCopyByTags ||
