...
namespace YooAsset.Editor
{
    public class TaskGetBuildMap
    {
        ...
        public BuildMapContext CreateBuildMap(BuildParameters buildParameters)
        {
            ...
            var allBuildAssetInfos = new Dictionary<string, BuildAssetInfo>(1000);
            ...
            foreach (var collectAssetInfo in allCollectAssets)
            {
                ...
                if (collectAssetInfo.CollectorType != ECollectorType.MainAssetCollector)
                    if (collectAssetInfo.AssetTags.Count > 0)
                    {
                        ...
                        var warning = BuildLogger.GetErrorMessage(ErrorCode.RemoveInvalidTags,
                            $"Remove asset tags that don't work, see the asset collector type : {collectAssetInfo.AssetInfo.AssetPath}");
                        BuildLogger.Warning(warning);
                    }

                var buildAssetInfo = new BuildAssetInfo(collectAssetInfo.CollectorType, collectAssetInfo.BundleName,
                    collectAssetInfo.Address, collectAssetInfo.AssetInfo);
            ...
            foreach (var collectAssetInfo in allCollectAssets)
            {
                var bundleName = collectAssetInfo.BundleName;
            ...
            if (collectResult.Command.AutoCollectShaders)
            {
                ...
                var shaderBundleName = shaderPackRuleResult.GetBundleName(collectResult.Command.PackageName,
                    collectResult.Command.UniqueBundleName);
            ...
            if (buildParameters.EnableSharePackRule)
            {
                ...
                foreach (var buildAssetInfo in allBuildAssetInfos.Values)
                    ...
                    {
                        var packRuleResult = GetShareBundleName(buildAssetInfo);
                        if (packRuleResult.IsValid())
                        {
                            var shareBundleName = packRuleResult.GetShareBundleName(collectResult.Command.PackageName,
                                collectResult.Command.UniqueBundleName);
            ...
            // 9. 移除不参与构建的资源
            var removeBuildList = new List<BuildAssetInfo>();
            foreach (var buildAssetInfo in allBuildAssetInfos.Values)
                if (buildAssetInfo.HasBundleName() == false)
                    removeBuildList.Add(buildAssetInfo);
            foreach (var removeValue in removeBuildList) allBuildAssetInfos.Remove(removeValue.AssetInfo.AssetPath);
            ...
            {
                var message =
                    BuildLogger.GetErrorMessage(ErrorCode.PackAssetListIsEmpty, "The pack asset info is empty !");
                throw new Exception(message);
            }

            foreach (var assetInfo in allPackAssets) context.PackAsset(assetInfo);
        ...
        private void RemoveZeroReferenceAssets(BuildMapContext context, List<CollectAssetInfo> allCollectAssets)
        {
            ...
            // 2. 获取所有主资源的依赖资源集合
            var allDependAsset = new HashSet<string>();
            ...
            // 3. 找出所有零引用的依赖资源集合
            var removeList = new List<CollectAssetInfo>();
            foreach (var collectAssetInfo in allCollectAssets)
            {
            ...
            {
                var warning = BuildLogger.GetErrorMessage(ErrorCode.FoundUndependedAsset,
                    $"Found undepended asset and remove it : {removeValue.AssetInfo.AssetPath}");
                BuildLogger.Warning(warning);
        ...
        ///     共享资源打包前置处理
        /// </summary>
        protected virtual void PreProcessPackShareBundle(BuildParameters buildParameters, CollectCommand command,
            Dictionary<string, BuildAssetInfo> allBuildAssetInfos)
        {
        }
        ...
        {
            var bundleName = Path.GetDirectoryName(buildAssetInfo.AssetInfo.AssetPath);
