...
namespace YooAsset.Editor
{
    ...
    {
        protected void CreateReportFile(BuildParametersContext buildParametersContext, BuildMapContext buildMapContext,
            ManifestContext manifestContext)
        {
            ...
            // 概述信息
            {
                ...
                var packageInfo = PackageInfo.FindForAssembly(typeof(BuildReport).Assembly);
                if (packageInfo != null)
                    buildReport.Summary.YooVersion = packageInfo.version;
#endif
                buildReport.Summary.UnityVersion = Application.unityVersion;
                ...
                buildReport.Summary.EncryptionClassName = buildParameters.EncryptionServices == null
                    ? "null"
                    : buildParameters.EncryptionServices.GetType().FullName;
            ...
            foreach (var packageAsset in manifest.AssetList)
            {
                ...
                var reportAssetInfo = new ReportAssetInfo();
                ...
                reportAssetInfo.DependAssets =
                    GetDependAssets(buildMapContext, mainBundle.BundleName, packageAsset.AssetPath);
            ...
            {
                var reportBundleInfo = new ReportBundleInfo();
            ...
            // 序列化文件
            var fileName =
                YooAssetSettingsData.GetReportFileName(buildParameters.PackageName, buildParameters.PackageVersion);
        ...
        {
            var dependBundles = new List<string>(packageBundle.DependIDs.Length);
            foreach (var index in packageBundle.DependIDs)
            {
                var dependBundleName = manifest.BundleList[index].BundleName;
        ...
        {
            long fileBytes = 0;
            foreach (var packageBundle in manifest.BundleList) fileBytes += packageBundle.FileSize;
            return fileBytes;
        }
