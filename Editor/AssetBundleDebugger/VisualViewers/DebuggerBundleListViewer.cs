...
namespace YooAsset.Editor
{
    internal class DebuggerBundleListViewer
    {
        ...
        {
            var result = new List<DebugBundleInfo>(1000);
            foreach (var pakcageData in debugReport.PackageDatas)
            {
                var tempDic = new Dictionary<string, DebugBundleInfo>(1000);
        ...
        private void BundleListView_onSelectionChange(IEnumerable<object> objs)
        {
            ...
            {
                var bundleInfo = item as DebugBundleInfo;
        ...
        {
            var providers = _usingListView.itemsSource as List<DebugProviderInfo>;
            var providerInfo = providers[index];
        ...
        {
            var source = new List<DebugProviderInfo>();
